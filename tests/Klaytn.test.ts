...
const test_sender_privateKey = "insert private key here";
const test_receiver_address = "0x0EE56B604c869E3792c99E35C1C424f88F87dC8a";

const validateSenderFromSignedTxnAndExpectedPublicKey = (
  signedTxn: AbstractTransaction,
  expectedPublicKey: string = test_sender_publicKey
) => {
  let signedRawTx = signedTxn.getRawTransaction();
  const recoveredPubkey = caver.transaction.recoverPublicKeys(signedRawTx);
  expect(recoveredPubkey[0]).toEqual(test_sender_publicKey);
};
const validateSenderFromSignedTxnAndExpectedAddress = (
  signedTxn: AbstractTransaction,
...
test("signValueTransfer with display", async () => {
  ...
  validateSenderFromSignedTxnAndExpectedPublicKey(
    signedTxn,
    test_sender_address
  );
});
...
//   expect(address).toEqual("0x6694d467b419b36fb719E851cD65d54205Df7555");
// });
...
//   expect(address).toEqual("0x20CC345eeA7F306c326d57aBDAb4445532e3d4ea");
// });
...
//   const { signature, signedTxn } = await klaytn.signLegacyTransaction(
//     txnToSign
//   );

//   validateSenderFromSignedTxnAndExpectedPublicKey(signedTxn, test_sender_address);
// });
...
//   const { signature, signedTxn } = await klaytn.signValueTransferMemo(
//     txnToSign
//   );
...
//   const { signature, signedTxn } = await klaytn.signSmartContractDeploy(
//     txnToSign
//   );
...
//   const { signature, signedTxn } = await klaytn.signSmartContractExecution(
//     txnToSign
//   );

//   const expectedSig =
//     "56efc3f267021a912109919202b8e74e1ddf474486f43d0880bbfa1c1bd54b44f62881888f136d40a9b8e02a792bbf175284fe736ad470fb551ba57751f81717a3";
//   let signedRawTx = signedTxn.getRawTransaction();
...
//   const transport = await openTransportReplayer(
//     RecordStore.fromString(`
//         => e03800003e058000002c80002019800000000000000000000000e8a2e13819850ba43b7400830493e0946e93a3acfbadf457f29fb0e57fa42274004c32ea8220198080
//         <= 550548fd3f7d067c6fa423d44ff2acfc468dd787af39d6cd0ef23ea7e98818e96a14227429eff8e3d3fdfc57a6cf09659ee74ce058a893aa1143234b0b1ec906a29000
//     `)
//   );
//   const klaytn = new Klaytn(transport);
...
//     chainId: 8217,
//   });
//   console.log("txnToSign:", txnToSign);
//   const { signature, signedTxn } = await klaytn.signCancel(txnToSign);
//   validateSenderFromSignedTxnAndExpectedAddress(signedTxn);
// });
...
//   const { signature, signedTxn } = await klaytn.signFeeDelegatedValueTransfer(
//     txnToSign
//   );
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedValueTransferMemo(txnToSign);
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedSmartContractDeploy(txnToSign);
//   const expectedSig =
//     "56efc3f267021a912109919202b8e74e1ddf474486f43d0880bbfa1c1bd54b44f62881888f136d40a9b8e02a792bbf175284fe736ad470fb551ba57751f81717a3";
//   let signedRawTx = signedTxn.getRawTransaction();
//   const recoveredTx = caver.klay.accounts.recoverTransaction(signedRawTx);
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedSmartContractExecution(txnToSign);
...
//   const { signature, signedTxn } = await klaytn.signFeeDelegatedCancel(
//     txnToSign
//   );
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedValueTransferWithRatio(txnToSign);
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedValueTransferMemoWithRatio(txnToSign);
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedSmartContractDeployWithRatio(txnToSign);
...
//   const { signature, signedTxn } =
//     await klaytn.signFeeDelegatedSmartContractExecutionWithRatio(txnToSign);
...
//   const { signature, signedTxn } = await klaytn.signFeeDelegatedCancelWithRatio(
//     txnToSign
//   );
